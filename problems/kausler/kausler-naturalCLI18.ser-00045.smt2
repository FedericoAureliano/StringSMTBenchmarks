(set-logic QF_S)
(set-option :produce-models true)

(declare-fun temp_297_322 () String)
(declare-fun s317 () String)
(declare-fun s307 () String)
(declare-fun temp1_297_312 () String)
(declare-fun temp2_297_317 () String)
(declare-fun temp2_297_307 () String)
(declare-fun temp2_297_312 () String)
(declare-fun temp2_297_322 () String)
(declare-fun temp1_297_322 () String)
(declare-fun s297 () String)
(declare-fun temp1_297_317 () String)
(declare-fun s312 () String)
(declare-fun s322 () String)
(declare-fun temp1_297_307 () String)

(assert (= (str.len temp1_297_317) (str.len s317) ) )
(assert (not (= s312 temp2_297_312) ) )
(assert (= (str.++ temp1_297_322 temp2_297_322) s297) )
(assert (= (str.++ temp1_297_307 temp2_297_307) s297) )
(assert (= (str.len temp1_297_307) (str.len s307) ) )
(assert (= (str.++ temp1_297_317 temp2_297_317) s297) )
(assert (= (str.len temp2_297_322) (str.len s322) ) )
(assert (= (str.++ temp_297_322 s322) s297) )
(assert (not (= s322 temp2_297_322) ) )
(assert (= (str.len temp2_297_312) (str.len s312) ) )
(assert (= (str.++ temp1_297_312 temp2_297_312) s297) )
(assert (not (= s317 temp1_297_317) ) )
(assert (not (= s307 temp1_297_307) ) )


(check-sat)
(get-model)
