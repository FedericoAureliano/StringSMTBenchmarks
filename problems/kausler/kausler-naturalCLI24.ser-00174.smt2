(set-logic QF_S)
(set-option :produce-models true)

(declare-fun s643 () String)
(declare-fun s658 () String)
(declare-fun s648 () String)
(declare-fun temp1_631_648 () String)
(declare-fun s653 () String)
(declare-fun s631 () String)
(declare-fun temp_631_658 () String)
(declare-fun temp2_631_653 () String)
(declare-fun temp1_631_653 () String)
(declare-fun temp2_631_643 () String)
(declare-fun temp2_631_648 () String)
(declare-fun temp1_631_643 () String)
(declare-fun temp1_631_658 () String)
(declare-fun temp2_631_658 () String)

(assert (= (str.++ temp1_631_648 temp2_631_648) s631) )
(assert (= (str.len temp2_631_648) (str.len s648) ) )
(assert (= (str.++ temp_631_658 s658) s631) )
(assert (= (str.++ temp1_631_643 temp2_631_643) s631) )
(assert (not (= s648 temp2_631_648) ) )
(assert (not (= s643 temp1_631_643) ) )
(assert (= (str.len temp1_631_643) (str.len s643) ) )
(assert (= (str.len temp2_631_658) (str.len s658) ) )
(assert (not (= s653 temp1_631_653) ) )
(assert (= (str.++ temp1_631_658 temp2_631_658) s631) )
(assert (= (str.++ temp1_631_653 temp2_631_653) s631) )
(assert (not (= s658 temp2_631_658) ) )
(assert (= (str.len temp1_631_653) (str.len s653) ) )


(check-sat)
(get-model)
